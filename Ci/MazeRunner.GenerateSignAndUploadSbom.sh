...
function parse_arguments() {
  ...
  if [[ ${skip_uploading} == "no" ]] && [[ -z ${sbom_signing_key_file_path} ]]; then
    echo "Specifying --sbom-signing-key-file-path is mandatory!"
    usage
    exit 1
  fi

  if [[ ${skip_uploading} == "no" ]] && [[ -z ${dependency_tracker_url} ]]; then
    echo "Specifying --dependency-tracker-url is mandatory!"
    usage
    exit 1
  fi

  if [[ ${skip_uploading} == "no" ]] && [[ -z ${dependency_tracker_api_key_file_path} ]]; then
...
function generate_sign_and_upload_sbom() {
  ...
  if [[ ${skip_uploading} == "yes" ]]; then
    return # skip uploading the SBOM
  fi
