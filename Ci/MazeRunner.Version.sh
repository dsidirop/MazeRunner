...
function usage(){
    echo "usage: ./MazeRunner.Version.sh [--master-branch master] [--develop-branch develop] [--major 1] [--minor 0] [--patch 0] [--revision 0] [--build-id 0] [--commit HEAD] [--branch branchname] [-o | --output version.txt] [-h | --help]"
    echo "parameters:"
...
log "MazeRunner_Version_Major:$major"
log "MazeRunner_Version_Minor:$minor"
log "MazeRunner_Version_Patch:$patch"
log "MazeRunner_Version_Suffix:$suffix"
log "MazeRunner_Version_Revision:$revision"
log "MazeRunner_Version_BuildId:$build_id"
log "MazeRunner_Version_Core:$version_core"
log "MazeRunner_Version_Extension:$version_extension"
log "MazeRunner_Version_Full:$version_full"
log "MazeRunner_Version_Assembly:$version_assembly"
log "MazeRunner_Version_BranchName:$branch_name"
log "MazeRunner_Version_ScriptCalled:true"

if [ "$output" != "" ]; then
    mkdir -p "$(dirname "$output")" && touch "$output" ;
    echo "MazeRunner_Version_Major=$major" > $output
    echo "MazeRunner_Version_Minor=$minor" >> $output
    echo "MazeRunner_Version_Patch=$patch" >> $output
    echo "MazeRunner_Version_Suffix=$suffix" >> $output
    echo "MazeRunner_Version_Revision=$revision" >> $output
    echo "MazeRunner_Version_BuildId=$build_id" >> $output
    echo "MazeRunner_Version_Core=$version_core" >> $output
    echo "MazeRunner_Version_Extension=$version_extension" >> $output
    echo "MazeRunner_Version_Full=$version_full" >> $output
    echo "MazeRunner_Version_Assembly=$version_assembly" >> $output
    echo "MazeRunner_Version_BranchName=$branch_name" >> $output
fi
